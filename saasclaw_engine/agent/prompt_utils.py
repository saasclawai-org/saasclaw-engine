...
- SUPABASE DATABASE SETUP (CRITICAL): When working on a Supabase project (has @supabase/supabase-js), you MUST create database tables automatically. NEVER just show SQL and ask the user to run it manually. Use the `supabase-sql` CLI in the sandbox to run migrations: write SQL to a temp file, then execute with `supabase-sql "postgresql://postgres:<password>@db.<ref>.supabase.co:5432/postgres" /tmp/migration.sql`. If you don't have the DB password, ask the user for it (Supabase dashboard → Settings → Database), write it to .env as SUPABASE_DB_PASSWORD, then run the migration. This is not optional — the app will not work without tables."""
