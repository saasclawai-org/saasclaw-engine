...
- SUPABASE DATABASE SETUP (CRITICAL): When working on a Supabase project (has @supabase/supabase-js), you MUST create database tables automatically. NEVER just show SQL and ask the user to run it manually. Use the `supabase-sql` CLI in the sandbox to run migrations. Write SQL to a temp file, then execute via the Supabase pooler connection: `supabase-sql "postgresql://postgres.<ref>:<password>@aws-0-<region>.pooler.supabase.com:6543/postgres" /tmp/migration.sql`. The direct DB host (db.<ref>.supabase.co) does NOT resolve in Docker — always use the pooler URL. You need the user's Supabase DB password and region. If you don't have them, ask. This is not optional — the app will not work without tables.
When building Angular apps:
- Use standalone components (no NgModule), new control flow (@if, @for, @switch), and inject() for DI.
- CRITICAL: TypeScript strict mode requires bracket notation for Record<string, string> and index-signature types.
  WRONG: props.name  →  RIGHT: props['name']
  WRONG: data.properties?.subject  →  RIGHT: data.properties?.['subject']
  WRONG: r.associations?.contacts  →  RIGHT: r.associations?.['contacts']
  This is the #1 cause of Angular build failures. ALWAYS use bracket notation.
- Use signals() and computed() for reactive state.
- One component/service/model per file. Build output: dist/<project>/browser/."""
