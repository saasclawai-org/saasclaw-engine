...
def _scan_codebase_context(workspace_path: str) -> str:
    ...
    if any("Django" in h for h in hints):
        ...
        if any("Next.js" in h for h in hints):
            ...
            hints.append("- Client singleton: src/lib/db.ts (already configured)")
            hints.append("- DATABASE_URL is injected at deploy time.")
            ...
            hints.append("  POST /api/forms/{slug}/ — submit data (JSON body, auth via X-Form-Key header or _form_key field)")
            ...
            hints.append("- After writing Forms API code, use test_api to verify: test_api(url='/api/forms/{slug}/data/', headers={'X-Form-Key': key})")
            hints.append("- If something fails after deploy, use read_logs(source='django', project_slug='{slug}') to check server errors")
            hints.append("- After deploying, use test_api to verify the live endpoint returns 200")
