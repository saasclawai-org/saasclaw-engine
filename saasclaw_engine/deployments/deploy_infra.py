...
def _ensure_nginx_static(service_name, domain, web_root, log_file=None, include_predator=False, extra_includes=None):
    ...
    # Match by service_name — only skip if THIS deploy's service_name is custom.
    custom_configs = [
        'traction-preview',  # Traction frontend has custom /api/ proxy to port 21019
    ]
    if service_name in custom_configs:
        # This deploy has a custom config — make sure it's enabled
        custom_path = Path('/etc/nginx/sites-available') / service_name
        custom_enabled = Path('/etc/nginx/sites-enabled') / service_name
        if custom_path.exists() and not custom_enabled.exists():
        ...
        if log_file:
            with log_file.open('a', encoding='utf-8') as h:
                h.write(f'Preserved custom nginx config: {service_name}\n')
        return True
